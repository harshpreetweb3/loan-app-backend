...
function rangeFromQuery(query) {
  const now = new Date();
  if (!query.range || query.range === 'overall') return { isOverall: true, start: new Date(0), end: endOfDay(now) };
  if (query.range === 'week') {
    const start = startOfDay(now);
    start.setDate(start.getDate() - start.getDay());
    return { isOverall: false, start, end: endOfDay(now) };
  }
  if (query.range === 'month') return { isOverall: false, start: new Date(now.getFullYear(), now.getMonth(), 1), end: endOfDay(now) };
  if (query.range === 'year') return { isOverall: false, start: new Date(now.getFullYear(), 0, 1), end: endOfDay(now) };
  ...
  return { isOverall: false, start: startOfDay(now), end: endOfDay(now) };
}

export const getDashboard = asyncHandler(async (req, res) => {
  const todayStart = startOfDay();
  const todayEnd = endOfDay();
  const { isOverall, start: rangeStart, end: rangeEnd } = rangeFromQuery(req.query);
  ...
  const rangeQuery = isOverall ? {} : { createdAt: { $gte: rangeStart, $lte: rangeEnd } };
  const dueOwnerQuery = {};
  ...
  const [totalBorrowers, totalLoans, totalActiveLoans, totalClosedLoans, overdueLoans, todaysDue, myCollections, dailyDueLoans, overdueInstallmentLoans, recentPayments, allPayments, loanTotals] = await Promise.all([
    Borrower.countDocuments({ ...(isAgent ? { createdBy: req.user._id } : {}), ...rangeQuery }),
    Loan.countDocuments({ ...ownerQuery, ...rangeQuery }),
    Loan.countDocuments({ ...ownerQuery, status: 'active', ...rangeQuery }),
    Loan.countDocuments({ ...ownerQuery, status: 'completed', ...rangeQuery }),
    ...
    Payment.find({ ...collectionOwnerQuery, ...rangeQuery }).populate('borrower').populate('collectedBy', 'name username').sort({ createdAt: -1 }).limit(8),
    Payment.aggregate([
      { $match: { ...collectionOwnerQuery, ...rangeQuery } },
      {
        $group: {
          _id: null,
    ...
    Loan.aggregate([{ $match: { ...ownerQuery, ...rangeQuery } }, { $group: { _id: null, totalDistributed: { $sum: '$loanAmount' } } }])
  ]);
